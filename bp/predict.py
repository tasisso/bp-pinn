import torch
import pytorch_lightning as pl
from models.deeponet import DeepONetLightning
from data.lightning import WaveformDataModule  # if using the native one
from torch.utils.data import DataLoader, TensorDataset
import numpy as np
import pandas as pd
from pathlib import Path 
from omegaconf import OmegaConf
from losses.NavierStokesPhysics import NavierStokes1DPhysicsLoss
from tqdm import tqdm
import os


MODEL_DICT = {
    "deeponet": DeepONetLightning,
}

def build_dataloader_from_numpy(inputs_list, batch_size=32, device="cuda"):
    # Example: inputs_list = [branch_input, trunk_input]
    # Each should be a NumPy array of shape [N, ...]
    tensors = [torch.tensor(arr, dtype=torch.float32, device=device) for arr in inputs_list]
    dataset = TensorDataset(*tensors)
    return DataLoader(dataset, batch_size=batch_size)

def initialize_buffer(pred_cols, static_cols, gt_cols, forecast_flag):
    buffer = {"t": []}
    for key in gt_cols:
        if key == "output":
            buffer["ABP"] = []
        elif key == "phi_ref":
            buffer["phi_ref"] = []
        elif key == "input":
            buffer["II"] = []
            buffer["PLETH"] = [] 
    for key in pred_cols:
        buffer[f"imputed_{key}"] = []
        if forecast_flag:
            buffer[f"forecasted_{key}"] = []
    for key in static_cols:
        buffer[f"imputed_{key}"] = []
    return buffer

def get_window(batch, batch_idx, pred_dict_cpu, pred_cols, static_cols, gt_cols, forecast_flag):
    """
    Extract values from batch[batch_idx]/pred_dict_cpu and build window, 
    splits model output into imputation and prediction. 

    Args:
    pred cols (list): any of ["P", "Q", "dPdt", "dQdt", "phi"] 
    static_cols (list): any of ["R1", "R2", "C", "A0", "Kr", "beta", "Pext", "rho"]
    gt_cols (list): any of ["input", "phi_ref", "output"]
    forecast_flag (bool): False when T==t_input
    """
    window = {}
    t_input = batch["t_input"].size(1)
    T = batch["t"].size(1)
    dt = batch["dt"][batch_idx]

    t = torch.arange(T) * dt
    start_timestamp = batch["start"][batch_idx]
    t_axis = start_timestamp + pd.to_timedelta(t.numpy(), unit='s')

    window["t"] = t_axis

    #Ground truth
    for key in gt_cols:
        arr = batch[key][batch_idx]
        if isinstance(arr, torch.Tensor):
            arr = arr.detach().cpu().numpy()
        if key == "output":
            window["ABP"] = arr
        elif key == "phi_ref":
            window["phi_ref"] = arr.squeeze(1)
        elif key == "input":
            window["II"] = arr[:, 0]
            window["PLETH"] = arr[:, 1]

    #Predictions
    for key in pred_cols:
        if key not in pred_dict_cpu:
            raise KeyError(f"Missing key {key} in sample output.")
        arr = pred_dict_cpu[key][batch_idx]
        if isinstance(arr, torch.Tensor):
            arr = arr.detach().cpu().numpy()
        if arr.ndim == 2 and arr.shape[-1] == 1:
            arr = np.squeeze(arr, axis=-1)
        
        window[f"imputed_{key}"] = arr[:t_input] #[0, 249)
        if forecast_flag:
            window[f"forecasted_{key}"] = arr[t_input:] #[249, 499)
    
    #Scalars
    for key in static_cols:
        if key not in pred_dict_cpu:
            raise KeyError(f"Missing key {key} in sample output.")
        arr = pred_dict_cpu[key][batch_idx]
        if isinstance(arr, torch.Tensor):
            arr = arr.detach().cpu().numpy()
        #arr = np.array([arr.item()] * t_input)
        arr = np.full(t_input, arr.item(), dtype=arr.dtype)
        window[f"imputed_{key}"] = arr

    return window

def add_preds(buffer, window, T, t_input, forecast_flag, state_dict):
    """
    Add a window to a running dataframe buffer dictionary.

    Args:
        buffer (dict): Per subject expanding dataframe buffer
        window (dict): Pre-processed window to add
        T (int): total steps of window
        t_input (t): chunk steps of window
        forecast_flag (bool): False when T==t_input
        state_dict (dict): track window state i.e. "start" or "end" or "_"
    """
    if forecast_flag:
        #First window
        if state_dict["pos"] == "start":
            for key, arr in window.items():
                if key in ["t", "ABP", "II", "PLETH", "phi_ref"] or "imp" in key:
                    buffer[key].extend(arr[:t_input])
                elif "for" in key:
                    #Pad first t_input with nans
                    buffer[key].extend([np.nan] * (t_input) + arr[:t_input].tolist()) #if T is odd, truncates forecast by 1
            #Update 
            state_dict["pos"] = "_"
        
        #Last window
        elif state_dict["pos"] == "end":
            for key, arr in window.items():
                if key in ["t", "ABP", "phi_ref"] or "for" in key:
                    buffer[key].extend(arr) #full window 
                elif key in ["II", "PLETH"] or "imp" in key: 
                    #Pad last T-t_input with nans
                    buffer[key].extend(arr.tolist() + [np.nan] * (T-t_input))
            #Update
            state_dict["pos"] = "start"

        else:
            for key, arr in window.items():
                buffer[key].extend(arr[:t_input]) #if T is odd, truncates forecast by 1
    else:
        for key, arr in window.items():
            buffer[key].extend(arr)

def streaming_inference(model, dataloader, save_path, pred_cols, static_cols, gt_cols):
    
    save_path = Path(save_path)
    save_path.mkdir(parents=True, exist_ok=True)

    def write_patient_data(df_buffer, patient_key):
      sid, hid = patient_key
      predfile = save_path / f"{sid}_{hid}_out.csv"
      df = pd.DataFrame(df_buffer)
      print(f"Writing {predfile}...")
      df.to_csv(predfile, index=False)

    #Check forecast
    forecast_flag = False
    if dataloader.dataset.forecast_duration > 0.0:
        forecast_flag = True
    
    #Initialize 
    previous_key = None
    previous_window = None
    state_dict = {"pos": "start"}
    T = None
    t_input = None 
    buffer = initialize_buffer(pred_cols, static_cols, gt_cols, forecast_flag)
    
    for batch in tqdm(dataloader, desc="Running inference and writing to file", total=None):

        batch_on_device = {
            k: v.to(device) if torch.is_tensor(v) else v
            for k, v in batch.items()
        }
        pred_dict = model(batch_on_device)  # returns dict of predictions
        pred_dict_cpu = {k: v.detach().cpu() for k, v in pred_dict.items()}
        batch_size = next(iter(pred_dict_cpu.values())).shape[0]  # assumes all preds same B

        t_input = batch["t_input"].size(1)
        T = batch["t"].size(1)
        
        #Flatten per sample
        for i in range(batch_size):
            #Extract current key and window data
            patient_key = (batch["subject_id"][i], batch["hadm_id"][i])
            window = get_window(batch, i, pred_dict_cpu, pred_cols, static_cols, gt_cols, forecast_flag)

            #Encounter new patient key -> write out previous buffer
            if previous_key is not None and patient_key != previous_key:
                state_dict["pos"] = "end"
                add_preds(buffer, previous_window, T, t_input, forecast_flag, state_dict)
                write_patient_data(buffer, previous_key)
                print("Done. Resetting buffer.")

                #Reset buffer, step window pointer 
                buffer = initialize_buffer(pred_cols, static_cols, gt_cols, forecast_flag)
                previous_key = patient_key
                previous_window = window
                continue

            #Add prev window to buffer 
            if previous_window is not None:
                add_preds(buffer, previous_window, T, t_input, forecast_flag, state_dict)
            previous_key = patient_key
            previous_window = window

    #Write out final patient
    if previous_window is not None:
        state_dict["pos"] = "end"
        add_preds(buffer, previous_window, T, t_input, forecast_flag, state_dict)
        write_patient_data(buffer, previous_key)

if __name__ == "__main__":
    """
    usage: 
    python predict.py \
        --config=../experiments/full-v3/from_scratch.yaml \
        --ckpt=../experiments/full-v3/checkpoints/best-v1.ckpt \
        --batch_size=1 \
        --gpu=3
    """
    import argparse
    parser = argparse.ArgumentParser(description="Run DeepONet Inference")
    parser.add_argument("--ckpt", type=str, default=None)
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--use_numpy", action="store_true", default=False, help="Use raw numpy inputs instead of datamodule")
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--gpu", type=int, default=None, help="GPU index to use (overrides config)")
    
    args = parser.parse_args()
    cfg = OmegaConf.load(args.config)

    save_dir = cfg.train_config.get("checkpoint_dir", ".")
    print(f"Save dir: {save_dir}")
    ckpt_path = args.ckpt or cfg.train_config.get("saved_model", None)
    # Override GPU if specified in CLI
    gpu_index = args.gpu if args.gpu is not None else cfg.train_config.get("gpu", 0)
    device = torch.device(f"cuda:{gpu_index}" if torch.cuda.is_available() else "cpu")    # Load model
    # Override batch size if specified in CLI
    if args.batch_size > 1: # otherwise use default from config
        cfg.dataloader_config["test_batch_size"] = args.batch_size
    
    model = cfg.model_config["arch"]

    print("Initializing model")
    loss_fn = NavierStokes1DPhysicsLoss(**cfg.loss_config)
    model = DeepONetLightning(
            model_config=cfg.model_config,
            loss_fn=loss_fn,
            optimizer_config=cfg.optimizer_config,
            sample_size=cfg.train_config.get("sample_size", None)
        )
    ckpt_path = "../experiments/overfit/impute_model.pth"
    state_dict = torch.load(ckpt_path, map_location=device)
    model.load_state_dict(state_dict)
    model.to(device)
    model.inference_mode() # set sample size to 1, eval() and freeze()

    # Build dataloader
    if args.use_numpy:
        # NOT IMPLEMENTED
        # Example: hardcoded paths or numpy arrays (replace with actual data loading)
        branch_input = np.load("branch_inputs.npy")
        trunk_input = np.load("trunk_inputs.npy")
        test_loader = build_dataloader_from_numpy([branch_input, trunk_input], batch_size=args.batch_size)
    else:
        datamodule = WaveformDataModule(**cfg.dataloader_config)
        datamodule.setup()
        test_loader = datamodule.test_dataloader()
    
    print("Running inference")
    # Run inference
    streaming_inference(model, test_loader, f"{save_dir}/inference", 
                        pred_cols=["P", "Q", "dPdt", "dQdt", "phi"] ,
                        static_cols=["R1", "R2", "C", "A0", "Kr", "beta", "Pext", "rho"],
                        gt_cols=["input", "phi_ref", "output"])
    print("Done")